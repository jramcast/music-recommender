...
def find(songid, data_dir):
    global trackid_to_file
    global songid_to_trackid

    if not songid_to_trackid:
        songid_to_trackid = _load_song_to_track_map()

    if not trackid_to_file:
        trackid_to_file = _load_song_files(data_dir)

    try:
        trackid = songid_to_trackid[songid]
        filepath, songidx = trackid_to_file[trackid]
        return load_song(filepath, songid, songidx)
    except KeyError:
        raise SongNotFound
...
def load_song(filepath, song_id, songidx):
